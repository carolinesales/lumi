...
function DayCell({ date, outOfMonth, hoje, etapasDia, onSelect, selected }) {
  ...
  // Cor de fundo da célula quando há etapa 
...
// Cor do texto dependendo do estado
...
export default function MonthCalendar({ etapas = [], onOpenEtapa, proximasEtapas = [] }) {
  ...
  // Grid 
  const gridDomingo = useMemo(() => {
    const firstDay = new Date(ano, mes, 1)
    const start    = new Date(firstDay)
    start.setDate(start.getDate() - firstDay.getDay()) // volta até domingo
    const lastDay  = new Date(ano, mes + 1, 0)
    const cells = []
    const cur   = new Date(start)
    while (cur <= lastDay || cells.length % 7 !== 0) {
      cells.push({ date: new Date(cur), outOfMonth: cur.getMonth() !== mes })
      cur.setDate(cur.getDate() + 1)
    }
    return cells
  }, [ano, mes])
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          <button type="button" onClick={irAnterior} aria-label="Mês anterior"
            ...
            <i className="fa-solid fa-chevron-left text-[11px] text-lumi-gray" aria-hidden="true" />
          </button>
          <button type="button" onClick={irSeguinte} aria-label="Próximo mês"
            className="grid size-6 place-items-center rounded-full transition hover:bg-lumi-input">
            <i className="fa-solid fa-chevron-right text-[11px] text-lumi-gray" aria-hidden="true" />
          </button>
        </div>
      </div>
      ...
      <div>
        <div className="mb-2 grid grid-cols-7">
          {DIAS.map(d => (
            <span key={d} className="text-center font-nunito text-xs font-light text-lumi-secondary">{d}</span>
          ))}
        </div>
        <div className="grid grid-cols-7 gap-1">
          {gridDomingo.map(({ date, outOfMonth }, i) => (
